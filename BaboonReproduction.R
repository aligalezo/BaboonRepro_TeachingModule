## Baboon Reproduction Workshop
## Ali Galezo and Emily Levy
...
## The column "cycle_day" identifies the day of the female's sexual cycle that the fecal sample was collected.
...
class(bab$female)                   # We want R to recognize unique females as a categorical variable instead of as integers.
...
# 5. the proportion of cases where the female consorted with a non-alpha male on that cycle day.
...
bab_new <-
  bab %>%
  group_by(female) %>% # modify this line so it GROUPS BY cycle_day, then run the code
  summarize(n = n(),                                                   # count the number of fecal samples represented for each cycle day
...
# Run the code below:
ggplot(data = bab_new,
       aes(x = cycle_day,                                   # this indicates what the X-axis variable is
           y = swelling_size_mean,                          # this indicates what the Y-axis variable is
...
# Now, let's modify the above code to make a new plot.
# Instead of plotting the probability of a consorting with an ALPHA MALE on the y-axis, we want to plot the probability of consorting with a NON-ALPHA MALE on the y-axis.
# Try modifying the code above to accomplish this.
# Don't forget to change the y-axis label on line 104.
# Once your plot looks right, you can save it to your Desktop:
